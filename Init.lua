-- [[ strictly for lua 5.1 and luau + roblox ]]
...
do
		...
		local function __modImpl()
			...
			function EncryptStrings:CreateEncrypionService()
				...
				local function genCode()
					-- All numeric constants are baked as literals directly into the
					-- function bodies so the renamer has zero upvalue names to collide
					-- across the two EncryptStrings passes.
					...
					-- DECRYPT: every constant baked in as a literal
	...
	do
		local function __modImpl()
			...
			function DynamicXOR:apply(ast, pipeline)
				...
				-- nodeMap: original node object -> {index, neg}
				local nodeMap = {}
            ...
            do
				...
				-- Inject the decode block at the top of the script
				...
				-- Now resolve dtName as a global variable reference so we can
				-- build proper AST index expressions.
				-- We parse a reference expression to get the correct scope/id.
				local refExpr = nil
				do
				...
				-- Second pass: replace every original NumberExpression node
				-- that was encoded with  dtName[index]  (negated if needed).
				visitast(ast, nil, function(node)
					local info = nodeMap[node]
					if not info then
						return
					end
					-- Build: dtName[info.index]
	...
	do
		local function __modImpl()
			...
			local function buildRuntimeCode(roundKeys, rounds, funcName)
				-- bake round keys and tweaks as literals so the renamer has no
				-- upvalue names to accidentally collide with the EncryptStrings blocks
				local tweaks = {1,3,7,13,31,61,127,251}
				local code = "do\n"
				-- _bxor: fully self-contained, no upvalues
				...
				code = code .. "end\n"
				-- _dec_byte: round keys and tweaks inlined as literals via if/elseif chain
			...
			local function transformBody(stmts, poolSize, scope)
				...
				for i = 1, n do
					...
					if assignNode.kind == AstKind.AssignmentStatement then
						if assignNode.rhs and assignNode.rhs[1] then
							funcExpr = assignNode.rhs[1]
						elseif assignNode.values and assignNode.values[1] then
	...
	do
		local function __modImpl()
			...
			-- Luau only reliably dispatches __add and __sub metamethods on tables.
			-- __pow, __mul, __div, __mod all silently fail or error in Luau when
			-- the LHS is a table, producing a number-value crash downstream.
			local OPERATORS = {
				{ sym = "+", mm = "__add" },
				{ sym = "-", mm = "__sub" },
			}
			...
			local function flattenFunctionNode(node)
				...
				local function buildIfChain(entries, i)
					...
					if dispatchStmt.kind == AstKind.LocalVariableDeclaration then
						dispatchStmt = {
							kind = AstKind.AssignmentStatement,
							lhs = dispatchStmt.variables,
							rhs = dispatchStmt.values or {},
						}
					elseif dispatchStmt.kind == AstKind.LocalFunctionDeclaration then
						dispatchStmt = {
							kind = AstKind.AssignmentStatement,
							lhs = { { kind = AstKind.Identifier, name = dispatchStmt.name } },
							rhs = {
								{
					...
					local stateAssign = {
						kind = AstKind.AssignmentStatement,
						lhs = { { kind = AstKind.Identifier, name = "state" } },
						rhs = { { kind = AstKind.NumberExpression, value = entry.next } },
					}
					local ifNode = {
						kind = AstKind.IfStatement,
						condition = {
				...
				local whileNode = {
					kind = AstKind.WhileStatement,
					condition = { kind = AstKind.BooleanExpression, value = true },
					body = {
						kind = AstKind.Block,
	...
	do
		local function __modImpl()
			...
			-- Build a balanced binary-search if-tree dispatching on the counter variable.
			-- states is a 0-indexed table of statement-list arrays.
			...
			function BootstrapObfuscator:apply(ast, pipeline)
				...
				-- Separate return statement — it lives outside the loop
				...
				-- Build outer scope for the state machine
				...
				-- Convert 1-indexed buckets to 0-indexed states table
				local states = {}
	...
	do
		local function __modImpl()
			return {
				...
				BootstrapObfuscator = __DARKLUA_BUNDLE_MODULES.P(),
			}
		end
		function __DARKLUA_BUNDLE_MODULES.K()
			local v = __DARKLUA_BUNDLE_MODULES.cache.K
